...
def download_youtube_video(url, quality_choice, output_path="video"):
    try:
        ...
        ydl_opts = {
            ...
            # ---- Timeout / retry settings for slow internet ----
            'socket_timeout': 1800,       # 30 minutes (in seconds) before a stalled connection is dropped
            'retries': 50,                # retry a failed download chunk up to 50 times
            'fragment_retries': 50,       # retry failed fragments (for DASH/HLS streams)
            'extractor_retries': 10,      # retry if metadata extraction fails
            'file_access_retries': 10,    # retry if writing to disk fails
